...
import project.dto.QMainItemDto;
import project.entity.Item;
import project.entity.QItem;
import project.entity.QItemImg;

import javax.persistence.EntityManager;
import java.time.LocalDateTime;
import java.util.List;

public class ItemRepositoryCustomImpl implements ItemRepositoryCustom {  // ItemRepositoryCustom 인터페이스 상속

    // 동적으로 쿼리를 생성하기 위해 JPAQueryFactory 클래스 사용
    private JPAQueryFactory queryFactory;

    // 생성자로 JPAQueryFactory를 이용해 EntityManager 주입
    public ItemRepositoryCustomImpl(EntityManager em) {
        this.queryFactory = new JPAQueryFactory(em);
    }

    /* 상품 판매 상태 조건(ItemSellStatus)이 전체(null)일 경우 null을 return하고
    ...
    /*
     * ItemSearchDto.searchDataType의 값에 따라서 dateTime의 값을 이전 시간의 값으로 세팅 후 해당 시간 이후로 등록된 상품만 조회합니다.
    ...
    //검색어가 null이 아니면 상품명에 해당 검색어가 포함되ㅣ는 상품을 조회하는 조건을 반환합니다.
